...
export class ActivitiesManager {
  ...
  render() {
    if (!this.container) {
      // Intentar ID primero si está disponible, sino clase para robustez
      this.container = document.querySelector(".activities-container ul");
    }
    ...
    if (this.activities.length === 0) {
      this.container.innerHTML =
        '<p style="text-align: center; color: var(--text-light); padding: 2rem;">No tienes actividades aún. ¡Crea tu primera actividad!</p>';
    ...
    this.activities.forEach((activity) => {
      ...
      activityItem.className = `activity-card ${
        activity.completada ? "completed" : "pending"
      }`;

      activityItem.innerHTML = `
                <div class="activity-priority ${
                  activity.prioridad || "medium"
                }">
                    <i class="${
                      priorityIcons[activity.prioridad] || priorityIcons.medium
                    }"></i>
                </div>
                <div class="activity-content">
                    ...
                    <div class="activity-meta">
                        <span class="activity-date"><i class="fas fa-calendar"></i> ${this.formatDate(
                          activity.fecha
                        )}</span>
                        ${
                          activity.hora
                            ? `<span class="activity-time"><i class="fas fa-clock"></i> ${activity.hora}</span>`
                            : ""
                        }
                        <span class="activity-category ${
                          activity.categoria || "personal"
                        }">${activity.categoria || "Personal"}</span>
                    </div>
                </div>
                <div class="activity-status">
                    <label class="activity-checkbox">
                        <input type="checkbox" ${
                          activity.completada ? "checked" : ""
                        } data-action="toggle">
      ...
      activityItem
        ...
        .addEventListener("change", () => this.toggleActivity(activity.id));
      activityItem
        .querySelector('button[data-action="edit"]')
        .addEventListener("click", () => this.prepareEdit(activity));
      activityItem
  ...
  async deleteActivity(id) {
    if (!confirm("Esta actividad se eliminará permanentemente. ¿Continuar?"))
      return;
  ...
  prepareEdit(activity) {
    ...
    if (form) {
      ...
      if (form.elements.description)
        form.elements.description.value = activity.descripcion;
      ...
      if (form.elements.priority)
        form.elements.priority.value = activity.prioridad;
      if (form.elements.category)
        form.elements.category.value = activity.categoria;
    }
