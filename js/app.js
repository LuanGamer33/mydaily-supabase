...
class App {
  constructor() {
    ...
    this.lists = new ListsManager(this.ui);
  ...
  async init() {
    ...
    await this.auth.init();
      ...
      if (!sessionStorage.getItem('sessionWelcomeShown')) {
          this.ui.showToast("Sesión detectada", "success");
          sessionStorage.setItem('sessionWelcomeShown', 'true');
      }
  ...
  async loadDashboardData() {
    ...
    if (motivationTime)
      motivationTime.textContent = `Actualizado: ${motivation.time}`;
  }
  ...
  renderDashboardStats(notes, habits, events, activities) {
    ...
    const pendingActivitiesEl = document.getElementById(
      "pending-activities-stat",
    );
    if (pendingActivitiesEl)
      pendingActivitiesEl.textContent = pendingActivities;

    // Today specific
    const todayEvents = events.filter(
      (e) => new Date(e.fecha).toDateString() === today,
    ).length;
    ...
    const todayActivities = activities.filter(
      (a) => new Date(a.fecha).toDateString() === today && !a.completada,
    ).length;
  ...
  renderDashboardPreviews(notes, habits, events, activities) {
    ...
    renderSection(notes, "notes-carousel", "Sin notas recientes", (note) => {
      ...
      div.innerHTML = `
            ...
            `;
      div.onclick = () => (window.location.href = "notes.php");
      return div;
    });
    ...
    renderSection(habits, "habits-carousel", "Sin hábitos activos", (habit) => {
      ...
      div.innerHTML = `
                ...
                <span>${habit.nom}</span>
            `;
      div.onclick = () => (window.location.href = "habits.php");
      return div;
    });
    ...
    renderSection(
      ...
      (event) => {
        ...
        div.innerHTML = `
            ...
            `;
        div.onclick = () => (window.location.href = "events.php");
        return div;
      },
    );
    ...
    renderSection(
      ...
      (activity) => {
        ...
        div.innerHTML = `
            ...
            `;
        div.onclick = () => (window.location.href = "activities.php");
        return div;
      },
    );
  }
      ...
      if (hasEvent || hasActivity) {
        ...
        if (hasEvent && hasActivity) {
          indicator.style.background =
            "linear-gradient(45deg, #ff9800 50%, #2196f3 50%)";
        } else if (hasEvent) {
